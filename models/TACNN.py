...
def CNN_text_processer(input_u, input_i, user_vocab_size, item_vocab_size, embed_word_dim, random_seed,
    ...
    xu = Embedding(user_vocab_size, embed_word_dim, embeddings_initializer=Constant(
        initW_u), name='user_text_embed')(input_u)
    xi = Embedding(item_vocab_size, embed_word_dim, embeddings_initializer=Constant(
        initW_i), name='item_text_embed')(input_i)

    xu = Lambda(reshape_tensor, arguments={'shape': [-1, embed_word_dim, review_len_u, 1]})(xu)
    xi = Lambda(reshape_tensor, arguments={'shape': [-1, embed_word_dim, review_len_i, 1]})(xi)
    # xu = Lambda(tf.reshape)(xu, [-1, embed_word_dim, review_len_u, 1])
    # xi = Lambda(tf.reshape)(xi, [-1, embed_word_dim, review_len_i, 1])

    xu = Conv2D(num_filters, kernel_size=(embed_word_dim, filter_size), padding='valid', activation='relu',
                 use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', name='user_text_conv')(xu)
    xi = Conv2D(num_filters, kernel_size=(embed_word_dim, filter_size), padding='valid', activation='relu',
                 use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros', name='item_text_conv')(xi)

    xu = Lambda(reshape_tensor, arguments={'shape': [-1, xu.shape[-2], xu.shape[-1]]})(xu)
    xi = Lambda(reshape_tensor, arguments={'shape': [-1, xi.shape[-2], xi.shape[-1]]})(xi)
    # xu = tf.reshape(xu, [-1, xu.shape[-2], xu.shape[-1]])
    # xi = tf.reshape(xi, [-1, xi.shape[-2], xi.shape[-1]])

    xu = MaxPooling1D(pool_size=review_len_u - 2, padding='valid', name='user_text_pool')(xu)
    xi = MaxPooling1D(pool_size=review_len_i - 2, padding='valid', name='item_text_pool')(xi)

    xu = Lambda(reshape_tensor, arguments={'shape': [-1, review_num_u, num_filters]})(xu)
    xi = Lambda(reshape_tensor, arguments={'shape': [-1, review_num_i, num_filters]})(xi)
    # xu = tf.reshape(xu, [-1, review_num_u, num_filters])
    # xi = tf.reshape(xi, [-1, review_num_i, num_filters])
...
def weighted_sum(out_u, xu, out_i, xi, dropout_keep_prob, random_seed):


    feas_u = Multiply()([out_u, xu])
    feas_u = Lambda(tf.reduce_sum, arguments={'axis': 1})(feas_u)
    feas_i = Multiply()([out_i, xi])
    feas_i = Lambda(tf.reduce_sum, arguments={'axis': 1})(feas_i)
    # feas_u = tf.reduce_sum(Multiply()([out_u, xu]), axis=1)
    # feas_i = tf.reduce_sum(Multiply()([out_i, xi]), axis=1)
...
def DeepRecSys(l2_reg_lambda, random_seed, dropout_keep_prob, embed_word_dim, embed_id_dim,
    ...
    xu, xi = CNN_text_processer(input_u, input_i, user_vocab_size, item_vocab_size, embed_word_dim, random_seed,
                                  num_filters, filter_size, review_num_u, review_len_u, review_num_i, review_len_i,
                                  initW_u, initW_i)
    out_u, out_i = attention_weights(input_uid, input_iid, xu, xi,
                                     user_num, item_num, embed_id_dim, random_seed,
                                     attention_size, l2_reg_lambda)
    feas_u, feas_i = weighted_sum(out_u, xu, out_i, xi, dropout_keep_prob, random_seed)
